...
# Finding churn rate:
def ChurnRateDetect(request):
    if request.method == 'POST':
        if request.POST.get('result_button'):
            ...
            results = Finder(name, location, emp_group, function, gender, tenure_group, experience, age, Maritial, Hiring, Promoted, Job)
            print(results)
...
def Finder(name, location, emp_group, function, gender, tenure_group,
    ...
    if name != "":
        ...
        filename = os.path.join(os.path.dirname(os.path.dirname(__file__)), r'webapp\finalized_model.pickle')
